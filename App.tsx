...
const App: React.FC = () => {
    ...
    const handleAdminSettingsUpdate = useCallback(async (updatedData: Partial<AdminSettings>) => {
        ...
        try {
            const settingsDocRef = doc(db, `artifacts/${appId}/admin/settings`);
    ...
    const deleteUserData = async (uid: string) => {
        ...
        const feedbackQuery = query(collection(db, `artifacts/${appId}/public/data/feedback`), where('userId', '==', uid));
