...
import { collection, getDocs, orderBy, query, addDoc, Timestamp, doc, setDoc, writeBatch, arrayUnion, where } from '@firebase/firestore';
import { db, appId } from '../../services/firebase';
import type { AppUser, ModalContent, BroadcastData, Feedback, AdminSettings } from '../../types';
import { LogOut, Send, Users, Activity, RefreshCw, UserCheck, UserX, Search, MessageCircle, Trash2, Award, Settings } from 'lucide-react';
    ...
    const fetchAllData = useCallback(async (tab: string) => {
        ...
        try {
            ...
            if (tab === 'broadcasts') {
                ...
                const q = query(broadcastsCollection, orderBy('createdAt', 'desc'));
                const querySnapshot = await getDocs(q);
                setBroadcasts(querySnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id } as BroadcastData)));
            }
            if (tab === 'feedback') {
                const feedbacksCollection = collection(db, `artifacts/${appId}/public/data/feedback`);
            ...
            confirmAction: async () => {
                ...
                try {
                    ...
                    for (const u of users) {
                        ...
                        const q = query(notifsRef, where('type', '==', 'admin'));
                    ...
                    broadcasts.forEach(b => {
                        batch.delete(doc(db, `artifacts/${appId}/public/data/broadcasts`, b.id));
                    });
                    await batch.commit();
